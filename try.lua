...
return setmetatable({}, { __call = function(_, userSettings) return LimbExtender.new(userSettings) end, __index = LimbExtender, })	return s
end

for k, v in pairs(user) do
    s[k] = v
    if k == "PREDICTION" or k == "SMOOTHING" then
        s[k] = tonumber(v) or DEFAULTS[k]
    end
end
...
-- new code for prediction and smoothing
local targetLimb = char:FindFirstChild(self._parent._settings.TARGET_LIMB)
if targetLimb then
    local targetPosition = targetLimb.Position
    local prediction = self._parent._settings.PREDICTION
    local smoothing = self._parent._settings.SMOOTHING

    -- predict the target's movement
    local predictedPosition = targetPosition + (targetLimb.Velocity * prediction)

    -- smooth the aim
    local smoothedPosition = predictedPosition:Lerp(targetPosition, smoothing)

    -- update the highlight position
    if self.highlight then
        self.highlight.Adornee = targetLimb
        self.highlight.CFrame = CFrame.new(smoothedPosition)
    end
end
