...
-- Add silent aim
if self._parent._settings.SILENT_AIM then
    local character = self.player.Character
    local humanoidRootPart = character:FindFirstChild("HumanoidRootPart")
    local camera = game.Workspace.CurrentCamera
    local targetPart = limb

    local function silentAim()
        if humanoidRootPart and camera and targetPart then
            local direction = (targetPart.Position - humanoidRootPart.Position).Unit
            camera.CFrame = CFrame.new(humanoidRootPart.Position, targetPart.Position)
        end
    end
...
function PlayerData:setupCharacter(char)
	...
	if self._parent._settings.BASEPART_TARGET then
        local basepart = char:FindFirstChild("BasePart")
        if basepart then
            self:modifyLimbProperties(basepart)
        end
    else
        local targetLimb = char:FindFirstChild(self._parent._settings.TARGET_LIMB)
        if targetLimb then
            self:modifyLimbProperties(targetLimb)
        end
    end
end
...
self._settings.SILENT_AIM = userSettings.SILENT_AIM or false
    self._settings.BASEPART_TARGET = userSettings.BASEPART_TARGET or false
